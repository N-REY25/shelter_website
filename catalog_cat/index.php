...
<html lang="ru">
    ...
    <body>
        ...
        <section class="welcome_section">
            ...
            <div class="wrapper">
                <div class="welcome">
                    <h2 class="w_h2">Каталог кошек</h2>
                </div>
            </div>
        </section>
        <section class="happy_section">
            <div class="wrapper">
                <div class="happy">
                    <h3>МЫ ОБЯЗАТЕЛЬНО ПОДБЕРЕМ ВАМ ДРУГА!</h3>
                    <div class="h_items">
                        <!-- AJAX ответ -->
                    </div>
                </div>
        ...
        <script>
            $.ajax({
                ...
                method: 'get',
                dataType: 'json',
                data: {query: 'catalog', filter: 'cat'},
                success: function(data) {
                    $('.h_items').empty();
                    let i = 0;
                    while (i < data['zoo'].length) {
                        $('.h_items').append('<div class="h_item"><img class="hi_img" src="/img/zoo/' + data["zoo"][i]["photo"] + '"><h4 class="hi_h4">' + data["zoo"][i]["name"] + '</h4><p class="hi_age">' + data["zoo"][i]["pole"] + ', ' + data['zoo'][i]['age'] + '</p><p class="hi_kom">' + data["zoo"][i]["komm"] + '</p></div>');
                        i++;
                    }
                }
            });
